...
MODEL_ID=1
bashCommand = "java -jar ./learnFFSM.jar -fm ./"+SPL_NAME+"/model.xml -mref "+prodAnalyzed[0]+" -updt "+prodAnalyzed[1]+" -out "+out_directory+"ffsm_"+str(MODEL_ID)+"_kiss.txt"
...
for NEXT_ID in prodOrder:
	...
	tmp.append(NEXT_ID)
	bashCommand = "java -jar ./learnFFSM.jar -fm ./"+SPL_NAME+"/model.xml -fref "+out_directory+"ffsm_"+str(MODEL_ID-1)+"_kiss.txt  -updt "+NEXT_ID+" -out "+out_directory+"ffsm_"+str(MODEL_ID)+"_kiss.txt"
	_writer.write(bashCommand)
	_writer.write("\n")
	process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
	output, error = process.communicate()	
