...
prodOrder=["{:05d}".format(x) for x in range(1,30+1)]
...
for idx_PROD_I in range(len(prodOrder)):
	for idx_PROD_J in range(idx_PROD_I,len(prodOrder)):
		...
		print("Running for "+PROD_I+"_"+PROD_J)
		# if(PROD_J==PROD_I): continue
		bashCommand = "java -cp ./learnFFSM.jar uk.le.ac.compare.CompareStructure ./"+SPL_NAME+"/products/products_all/"+PROD_I+"_kiss.txt ./"+SPL_NAME+"/products/products_all/"+PROD_J+"_kiss.txt -fm ./"+SPL_NAME+"/model.xml "+_ktr
